...
func TestRunComplete(t *testing.T) {
	...
	jobs <- job{
		metric: stats.NewCounter(stats.Opts{Name: "metric_1"}, nil),
		value:  1,
	}
	jobs <- job{
		metric: stats.NewCounter(stats.Opts{Name: "metric_1"}, nil),
...
func TestRunError(t *testing.T) {
	...
	jobs := make(chan job, 3)
	jobs <- job{
		metric: stats.NewGauge(stats.Opts{Name: "metric_1"}, nil),
		value:  1,
	}
	jobs <- job{
		metric: stats.NewCounter(stats.Opts{Name: "metric_1"}, nil),
...
func TestBackend(t *testing.T) {
	...
	b.Set(stats.NewGauge(stats.MakeOpts("metric_1", ""), nil), 1)
	b.Add(stats.NewCounter(stats.MakeOpts("metric_2", ""), nil), 2)
	b.Observe(stats.NewHistogram(stats.MakeOpts("metric_3", ""), nil), time.Second)
